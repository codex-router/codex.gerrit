...
- **智能体（Agent）**：
	- 默认选择返回列表中的第一项。
	- 若加载失败，回退为 `codex`。
	- 若选择 `team`，后端会启用多智能体协作：非 `team` 智能体并行执行、内部辩论，并综合为一个最终答案。
- **模型（Model）**：
	- 默认选择返回列表中的第一项。
	- 所选模型会通过 `--model` 传给后端。
...
- 选择器区域还会显示上下文溢出保护（overflow guard）状态卡片。
- `Overflow guard: ready` 表示自动压缩兜底能力可用。
- `Overflow guard: monitoring` 表示当前请求正在监控模型上下文溢出。
- `Overflow guard: attention` 表示上下文溢出仍然失败（包括压缩重试后仍失败）；请缩短提示词/历史内容后重试。
